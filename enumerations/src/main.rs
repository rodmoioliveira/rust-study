...
// =======================================
// In this chapter we’ll look at enumerations, also referred to as enums. Enums allow you to define
// a type by enumerating its possible variants. First, we’ll define and use an enum to show how an
// enum can encode meaning along with data. Next, we’ll explore a particularly useful enum, called
// Option, which expresses that a value can be either something or nothing. Then we’ll look at how
// pattern matching in the match expression makes it easy to run different code for different
// values of an enum. Finally, we’ll cover how the if let construct is another convenient and
...
// major standards are used for IP addresses: version four and version six. These are the only
...
// values can only be one of its variants. Both version four and version six addresses are still
// fundamentally IP addresses, so they should be treated as the same type when the code is handling
...
fn main() {
    ...
    let m2 = Message::ChangeColor(10, 10, 10);
    m.call();
    m2.call();
}

...
fn main() {
    ...
    // you can become familiar with the concepts.

    // =======================================
    // Preventing Dangling References with Lifetimes
    // =======================================
    // The main aim of lifetimes is to prevent dangling references, which cause a program to reference
    // data other than the data it’s intended to reference. Consider the program in Listing 10-17,
    // which has an outer scope and an inner scope.

    // {
    //     let r;

    //     // Note: The examples in Listings 10-17, 10-18, and 10-24 declare variables without giving
    //     // them an initial value, so the variable name exists in the outer scope. At first glance,
    //     // this might appear to be in conflict with Rust’s having no null values. However, if we
    //     // try to use a variable before giving it a value, we’ll get a compile-time error, which
    ...
    // valid. Listing 10-18 shows the same code as Listing 10-17 but with annotations showing the
    ...
    // Listing 10-19 fixes the code so it doesn’t have a dangling reference and compiles without any
    // errors.
    ...
    // slices. This function will take two string slices and return a string slice. After we’ve
    // implemented the longest function, the code in Listing 10-20 should print The longest string is
    // abcd.

    // If we try to implement the longest function as shown in Listing 10-21, it won’t compile.

    // fn longest_(x: &str, y: &str) -> &str {
    //     if x.len() > y.len() {
    //         x
    //     } else {
    //         y
    ...
    // scopes as we did in Listings 10-18 and 10-19 to determine whether the reference we return will
    ...
    // example, let’s say we have a function with the parameter first that is a reference to an i32
    ...
    // Now let’s examine lifetime annotations in the context of the longest function. As with
    // generic type parameters, we need to declare generic lifetime parameters inside angle
    // brackets between the function name and the parameter list. The constraint we want to express
    // in this signature is that all the references in the parameters and the return value must
    // have the same lifetime. We’ll name the lifetime 'a and then add it to each reference, as
    ...
    // the function body. Rust can analyze the code within the function without any help. However,
    // when a function has references to or from code outside that function, it becomes almost
    // impossible for Rust to figure out the lifetimes of the parameters or return values on its
    // own. The lifetimes might be different each time the function is called. This is why we need
    // to annotate the lifetimes manually.
    ...
    // references that have different concrete lifetimes. Listing 10-23 is a straightforward example.
    ...
    // scope. Run this code, and you’ll see that the borrow checker approves of this code; it will
    // compile and print The longest string is long string is long.
    ...
    //        --> src/main.rs:251:44
    //         |
    //     251 |         result = longest(string1.as_str(), string2.as_str());
    //         |                                            ^^^^^^^ borrowed value does not live long enough
    //     252 |     }
    //         |     - `string2` dropped here while still borrowed
    //     253 |     println!("The longest string is {}", result);
    //         |                                          ------ borrow later used here

    // The error shows that for result to be valid for the println! statement, string2 would need
    // to be valid until the end of the outer scope. Rust knows this because we annotated the
    // lifetimes of the function parameters and return values using the same lifetime parameter 'a.
    ...
    // passed in. Therefore, the borrow checker disallows the code in Listing 10-24 as possibly having
    ...
    // type, but not for the parameter y, because the lifetime of y does not have any relationship with
    ...
    // a dangling reference because the value will go out of scope at the end of the function. Consider
    ...
    // lifetime of the parameters at all.
    ...
    // So far, we’ve only defined structs to hold owned types. It’s possible for structs to hold
    // references, but in that case we would need to add a lifetime annotation on every reference in
    // the struct’s definition. Listing 10-25 has a struct named ImportantExcerpt that holds a string
    // slice.
    ...
    // This struct has one field, part, that holds a string slice, which is a reference. As with
    // generic data types, we declare the name of the generic lifetime parameter inside angle
    // brackets after the name of the struct so we can use the lifetime parameter in the body of
    // the struct definition. This annotation means an instance of ImportantExcerpt can’t outlive
    ...
    // in Listing 4-9, which is shown again in Listing 10-26, that compiled without lifetime
    // annotations.
    ...
    // the lifetime of the remaining references should be. In this case, instead of guessing, the
    // compiler will give you an error that you can resolve by adding the lifetime annotations that
    ...
    // The first rule is that each parameter that is a reference gets its own lifetime parameter. In
    ...
    // The second rule is if there is exactly one input lifetime parameter, that lifetime is assigned
    ...
    // parameters. This third rule makes methods much nicer to read and write because fewer symbols are
    // necessary.

    // Let’s pretend we’re the compiler. We’ll apply these rules to figure out what the lifetimes of
    // the references in the signature of the first_word function in Listing 10-26 are. The signature
    // starts without any lifetimes associated with the references:
    ...
    // Listing 10-21: the compiler worked through the lifetime elision rules but still couldn’t figure
    ...
    let p = i.announce_and_return_part("aaaaaaaaaaaa");
    println!("PART: {}", p);

    // =======================================
    // The Static Lifetime
    // =======================================
    // One special lifetime we need to discuss is 'static, which means that this reference can live for
    // the entire duration of the program. All string literals have the 'static lifetime, which we can
    // annotate as follows:

    let _s: &'static str = "I have a static lifetime.";
    let _s2: &str = "I have a static lifetime.";
    let _s3: String = String::from("another");
    ...
    // that long, even if it could. Most of the time, the problem results from attempting to create a
    // dangling reference or a mismatch of the available lifetimes. In such cases, the solution is
    // fixing those problems, not specifying the 'static lifetime.
    ...
    let result = _longest_with_an_announcement(string1.as_str(), string2, "HERE!");

    // This is the longest function from Listing 10-22 that returns the longer of two string slices. But
    // now it has an extra parameter named ann of the generic type T, which can be filled in by any type
    // that implements the Display trait as specified by the where clause. This extra parameter will be
    // printed before the function compares the lengths of the string slices, which is why the Display
    // trait bound is necessary. Because lifetimes are a type of generic, the declarations of the lifetime
    // parameter 'a and the generic type parameter T go in the same list inside the angle brackets after
    // the function name.
    ...
    // trait bounds, and generic lifetime parameters, you’re ready to write code without repetition
    // that works in many different situations. Generic type parameters let you apply the code to
    // different types. Traits and trait bounds ensure that even though the types are generic, they’ll
    // have the behavior the code needs. You learned how to use lifetime annotations to ensure that
    // this flexible code won’t have any dangling references. And all of this analysis happens at
    ...
    // Chapter 17 discusses trait objects, which are another way to use traits. There are also more
    // complex scenarios involving lifetime annotations that you will only need in very advanced
    // scenarios; for those, you should read the Rust Reference. But next, you’ll learn how to write
    // tests in Rust so you can make sure your code is working the way it should.
}

fn main() {
    ...
    // =======================================
    // Object-oriented programming (OOP) is a way of modeling programs. Objects came from Simula in
    // the 1960s. Those objects influenced Alan Kay’s programming architecture in which objects
    // pass messages to each other. He coined the term object-oriented programming in 1967 to
    // describe this architecture. Many competing definitions describe what OOP is; some
    // definitions would classify Rust as object oriented, but other definitions would not. In this
    // chapter, we’ll explore certain characteristics that are commonly considered object oriented
    // and how those characteristics translate to idiomatic Rust. We’ll then show you how to
    // implement an object-oriented design pattern in Rust and discuss the trade-offs of doing so
    // versus implementing a solution using some of Rust’s strengths instead.

    // =======================================
    // Characteristics of Object-Oriented Languages
    // =======================================
    // There is no consensus in the programming community about what features a language must have
    // to be considered object oriented. Rust is influenced by many programming paradigms,
    // including OOP; for example, we explored the features that came from functional programming
    // in Chapter 13. Arguably, OOP languages share certain common characteristics, namely objects,
    // encapsulation, and inheritance. Let’s look at what each of those characteristics means and
    ...
    // colloquially referred to as The Gang of Four book, is a catalog of object-oriented design
    ...
    // provide methods on structs and enums. Even though structs and enums with methods aren’t
    // called objects, they provide the same functionality, according to the Gang of Four’s
    ...
    // Therefore, the only way to interact with an object is through its public API; code using the
    // object shouldn’t be able to reach into the object’s internals and change data or behavior
    // directly. This enables the programmer to change and refactor an object’s internals without
    ...
    // everything else is private. For example, we can define a struct AveragedCollection that has a
    ...
    // for us. Listing 17-1 has the definition of the AveragedCollection struct:
    ...
    // implementing add, remove, and average methods on the struct, as shown in Listing 17-2:

    // The public methods add, remove, and average are the only ways to access or modify data in an
    // instance of AveragedCollection. When an item is added to list using the add method or
    // removed using the remove method, the implementations of each call the private update_average
    ...
    // remove items to the list field directly; otherwise, the average field might become out of sync
    // when the list changes. The average method returns the value in the average field, allowing
    ...
    // have to change if it were modifying list directly.
    ...
    let mut my_colletion = AveragedCollection {
        list: vec![],
        average: 0.0,
    };
    ...
    // implementations. However, if you’re used to having inheritance in your programming toolbox, you
    // can use other solutions in Rust, depending on your reason for reaching for inheritance in the
    // first place.
    ...
    // different type. You can share Rust code using default trait method implementations instead,
    // which you saw in Listing 10-14 when we added a default implementation of the summarize method on
    // the Summary trait. Any type implementing the Summary trait would have the summarize method
    // available on it without any further code. This is similar to a parent class having an
    // implementation of a method and an inheriting child class also having the implementation of the
    // method. We can also override the default implementation of the summarize method when we
    // implement the Summary trait, which is similar to a child class overriding the implementation of
    ...
    // used in the same places as the parent type. This is also called polymorphism, which means that
    // you can substitute multiple objects for each other at runtime if they share certain
    // characteristics.
    ...
    // programming languages because it’s often at risk of sharing more code than necessary. Subclasses
    // shouldn’t always share all characteristics of their parent class but will do so with
    // inheritance. This can make a program’s design less flexible. It also introduces the possibility
    // of calling methods on subclasses that don’t make sense or that cause errors because the methods
    // don’t apply to the subclass. In addition, some languages will only allow a subclass to inherit

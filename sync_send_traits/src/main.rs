...
// However, two concurrency concepts are embedded in the language: the std::marker traits Sync and
// Send.
...
// this cannot be Send because if you cloned an Rc<T> value and tried to transfer ownership of the
// clone to another thread, both threads might update the reference count at the same time. For
// this reason, Rc<T> is implemented for use in single-threaded situations where you don’t want to
...
// Rc<T> value across threads unsafely. When we tried to do this in Listing 16-14, we got the error
// the trait Send is not implemented for Rc<Mutex<i32>>. When we switched to Arc<T>, which is Send,
// the code compiled.
...
// don’t have to implement those traits manually. As marker traits, they don’t even have any
// methods to implement. They’re just useful for enforcing invariants related to concurrency.

// Manually implementing these traits involves implementing unsafe Rust code. We’ll talk about
// using unsafe Rust code in Chapter 19; for now, the important information is that building new
// concurrent types not made up of Send and Sync parts requires careful thought to uphold the
// safety guarantees. “The Rustonomicon” has more information about these guarantees and how to
// uphold them.

// =======================================
// Summary
// =======================================
// This isn’t the last you’ll see of concurrency in this book: the project in Chapter 20 will use
// the concepts in this chapter in a more realistic situation than the smaller examples discussed
// here.

// As mentioned earlier, because very little of how Rust handles concurrency is part of the
// language, many concurrency solutions are implemented as crates. These evolve more quickly than
// the standard library, so be sure to search online for the current, state-of-the-art crates to
...
// checker ensure that the code using these solutions won’t end up with data races or invalid
